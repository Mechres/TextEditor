...
void enableRawMode(){
	...
	raw.c_iflag &= ~(BRKINT | INPCK | ISTRIP | ICRNL | IXON); // Input flag
	...
	if(fd != -1){
		if(ftruncate(fd, len) != -1){
			if(write(fd, buf, len) == len){
				...
				free(buf);
				E.dirty = 0;
				editorSetStatusMessage("%d bytes wrritten to disk", len);
				return;
			}
		}
	close(fd);
...
void editorDrawRows(struct abuf *ab){
	...
	for (y = 0; y < E.screenrows; y++){
		...
		if(E.numrows == 0 && y == E.screenrows / 3){ // Welcome screen fix
		char welcome[80];
